/**
  THIS FILE IS AUTO GENERATED
  Communicate changes to the dev team (e.g. via a Pull Request).
  Changes get lost otherwise.

  path: 'OpenSHC/Globals/DAT_SkirmishDefinedData.hpp'
*/

#pragma once

#include "OpenSHC/Game/Skirmish/SkirmishDefinedData.hpp"
namespace OpenSHC {

using OpenSHC::Game::Skirmish::SkirmishDefinedData;

// GLOBAL: STRONGHOLDCRUSADER 0x00B3EB34
// StructResolver::Instance<OpenSHC::Game::Skirmish::SkirmishDefinedData,11791156>::instance
MACRO_STRUCT_RESOLVER(SkirmishDefinedData, false, Address::SHC_3BB0A8C1_0x00B3EB34) DAT_SkirmishDefinedData;
} // namespace OpenSHC
