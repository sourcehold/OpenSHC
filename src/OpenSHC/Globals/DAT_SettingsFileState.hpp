/**
  THIS FILE IS AUTO GENERATED
  Communicate changes to the dev team (e.g. via a Pull Request).
  Changes get lost otherwise.

  path: 'OpenSHC/Globals/DAT_SettingsFileState.hpp'
*/

#pragma once

#include "OpenSHC/IO/SettingsFileState.hpp"

// GLOBAL: STRONGHOLDCRUSADER 0x00EE109A
// StructResolver::Instance<OpenSHC::IO::SettingsFileState,15601818>::instance

namespace OpenSHC {

using OpenSHC::IO::SettingsFileState;

MACRO_STRUCT_RESOLVER(SettingsFileState, false, Address::SHC_3BB0A8C1_0x00EE109A) DAT_SettingsFileState;
} // namespace OpenSHC
