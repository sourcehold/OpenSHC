...
#pragma pack(push, 1)
    ...
    public:
        ...
        void renderMultilineTextFromGroupWithBlend(eTextSectionsInt textOffsetIndex, int textNumInGroup, int xPos,
            int yPos, int maxWidth, uint color, int fontSize, int blendStrength);

        void renderMultilineTextFromGroup(eTextSectionsInt textOffsetIndex, int textNumInGroup, int xPos, int yPos,
            int maxWidth, uint color, int fontSize);

        void renderMultilineTextFromGroupWithShadow(eTextSectionsInt textOffsetIndex, int textNumInGroup, int xPos,
            int yPos, int maxWidth, uint color1, uint color2, int fontSize, int blendStrength);

        void renderMultilineTextFromGroupNoSpacing(eTextSectionsInt textOffsetIndex, int textNumInGroup, int xPos,
            int yPos, int maxWidth, uint color, int fontSize, int blendStrength);
        ...
        void setTextManagerClipRange(dword param_1, dword param_2);

        void resetTextManagerClipRange();

        void trimText(char* text, int allowedWidth, int fontSize);

        int getTextWidthTillCursor(char* text, int cursorIndex, int fontSize);

        void fillIntegerTextBuffer(int numberInt);

        char* getTextStringInGroupAtOffset(eTextSectionsInt offsetIndex, int numInGroup);

        int renderNumberDigitAtPlace(int numberToRenderUnk, int xPosUnk, int yPosUnk, int integerPartToRenderUnk,
            int bgr24, int digitSet, BOOL useCurrentXOffsetUnk);

        int getNumberGfxWidth(int param_1, int param_2);

        int getCharWidth(char char_, int fontSize);

        int computeTextWidth(char* text, int fontSize);

        int getIntegerTextWidth(int param_1, int param_2);

        void loadCRTex();

        void renderMultilineTextAtFontSize(
            char* text, int xPos, int yPos, int maxWidth, uint color, int fontSize, int blendStrength);

        void renderMultilineTextWithShadow(
            char* text, int xPos, int yPos, int maxWidth, uint color1, uint color2, int fontSize, int blendStrength);

        void renderMultilineWideTextWithShadow(LPWSTR _wideText, int xPos, int yPos, int maxWidth, uint color1,
            uint color2, int fontSize, int blendStrength);

        void renderPartialMultilineTextWrapper(
