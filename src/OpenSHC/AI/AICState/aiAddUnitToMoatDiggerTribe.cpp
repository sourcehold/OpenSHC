    ...
    void AICState::aiAddUnitToMoatDiggerTribe(int unitID)
    {
        ...
        if (tribeID == 0
            || DAT_TribesState::instance.tribes[tribeID].uid
                != DAT_GameState::instance.playerDataArray[owner].aiTribeUIDs[10]) {
            tribeID
                = MACRO_CALL_MEMBER(Map::Units::TribesState_Func::createTribeForPlayer, DAT_TribesState::ptr)(owner);
