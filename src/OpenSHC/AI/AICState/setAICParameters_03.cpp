#include "OpenSHC/AI/AIC/AITargetChoice.hpp"
#include "OpenSHC/AI/AICState.hpp"

namespace OpenSHC {
namespace AI {

    using AIC::AITC_ANY;
    using AIC::AITC_BALANCED;
    using AIC::AITC_CLOSEST;
    using AIC::AITC_GOLD;
    using AIC::AITC_PLAYER;

    // FUNCTION: STRONGHOLDCRUSADER 0x004C7520
    void AICState::setAICParameters_03(int param_1)
    {
        this->aics[param_1].minimumWheat = 0xffffffff;
        this->aics[param_1].minimumHop = 0xffffffff;
        this->aics[param_1].flagType = 0xc;
        this->aics[param_1].unknown002 = 0;
        this->aics[param_1].unknown001 = 0;
        this->aics[param_1].unknown003 = 0;
        this->aics[param_1].unknown004 = 0;
        this->aics[param_1].unknown005 = 0;
        this->aics[param_1].criticalPopularity = 5000;
        this->aics[param_1].lowestPopularity = 7000;
        this->aics[param_1].highestPopularity = 0x2134;
        this->aics[param_1].taxesMin = 3;
        this->aics[param_1].taxesMax = 0xb;
        this->aics[param_1].farm1 = 0x21;
        this->aics[param_1].populationPerFarm = 4;
        this->aics[param_1].unknown011 = 0x14;
        this->aics[param_1].maxFood = 400;
        this->aics[param_1].minimumApples = 10;
        this->aics[param_1].minimumCheese = 10;
        this->aics[param_1].minimumBread = 10;
        this->aics[param_1].tradeAmountFood = 5;
        this->aics[param_1].tradeAmountEquipment = 4;
        this->aics[param_1].aiRequestDelay = 0xc;
        this->aics[param_1].minimumGoodsRequiredAfterTrade = 100;
        this->aics[param_1].doubleRationsFoodThreshold = 0x3c;
        this->aics[param_1].maxWood = 0x46;
        this->aics[param_1].maxStone = 0x28;
        this->aics[param_1].maxResourceOther = 10;
        this->aics[param_1].maxEquipment = 10;
        this->aics[param_1].maxBeer = 5;
        this->aics[param_1].maxResourceVariance = 5;
        this->aics[param_1].recruitGoldThreshold = 200;
        this->aics[param_1].populationPerWoodcutter = 6;
        this->aics[param_1].populationPerQuarry = 0x19;
        this->aics[param_1].populationPerIronmine = 10;
        this->aics[param_1].populationPerPitchrig = 0x14;
        this->aics[param_1].maxQuarries = 2;
        this->aics[param_1].maxIronmines = 4;
        this->aics[param_1].maxWoodcutters = 8;
        this->aics[param_1].maxPitchrigs = 1;
        this->aics[param_1].maxFarms = 0xc;
        this->aics[param_1].buildInterval = 3;
        this->aics[param_1].resourceRebuildDelay = 0x32;
        this->aics[param_1].blacksmithSetting = 0x15;
        this->aics[param_1].fletcherSetting = 0x12;
        this->aics[param_1].poleturnerSetting = 0x13;
        this->aics[param_1].sellResource02 = 3;
        this->aics[param_1].sellResource04 = 7;
        this->aics[param_1].sellResource05 = 9;
        this->aics[param_1].sellResource06 = 0xe;
        this->aics[param_1].sellResource07 = 0x10;
        this->aics[param_1].sellResource08 = 0x11;
        this->aics[param_1].sellResource10 = 0x13;
        this->aics[param_1].sellResource11 = 0x14;
        this->aics[param_1].sellResource13 = 0x16;
        this->aics[param_1].sellResource15 = 0x18;
        this->aics[param_1].defWallPatrolRallyTime = 10;
        this->aics[param_1].defWallPatrolGroups = 4;
        this->aics[param_1].defSiegeEngineGoldThreshold = 100;
        this->aics[param_1].defSiegeEngineBuildDelay = 0x14;
        this->aics[param_1].unknown072 = 200;
        this->aics[param_1].unknown073 = 0x28;
        this->aics[param_1].recruitProbDefWeak = 100;
        this->aics[param_1].recruitProbDefDefault = 0x3c;
        this->aics[param_1].recruitProbDefStrong = 0x1e;
        this->aics[param_1].recruitProbRaidWeak = 0;
        this->aics[param_1].recruitProbRaidDefault = 0;
        this->aics[param_1].RecruitIntervalWeak = 1;
        this->aics[param_1].RecruitInterval = 1;
        this->aics[param_1].RecruitIntervalStrong = 4;
        this->aics[param_1].SortieUnitRangedMin = 4;
        this->aics[param_1].OuterPatrolRallyDelay = 0x14;
        this->aics[param_1].SortieUnitMeleeMin = 5;
        this->aics[param_1].RaidUnitsRandom = 5;
        this->aics[param_1].recruitProbRaidStrong = 0;
        this->aics[param_1].recruitProbAttackWeak = 0;
        this->aics[param_1].recruitProbAttackDefault = 0x28;
        this->aics[param_1].recruitProbAttackStrong = 0x46;
        this->aics[param_1].DefDiggingUnitMax = 0;
        this->aics[param_1].DefDiggingUnit = 0x18;
        this->aics[param_1].DefWalls = 0x1e;
        this->aics[param_1].DefTotal = 0x28;
        this->aics[param_1].OuterPatrolGroupsCount = 2;
        this->aics[param_1].OuterPatrolGroupsMove = 1;
        this->aics[param_1].RaidUnitsBase = 10;
        this->aics[param_1].HarassingSiegeEnginesMax = 2;
        this->aics[param_1].HarassingSiegeEngine1 = 0xbe;
        this->aics[param_1].RaidRetargetDelay = 3;
        this->aics[param_1].AttForceBase = 10;
        this->aics[param_1].AttForceRandom = 10;
        this->aics[param_1].SortieUnitRanged = 0x17;
        this->aics[param_1].DefUnit1 = 0x17;
        this->aics[param_1].DefUnit2 = 0x17;
        this->aics[param_1].DefUnit3 = 0x17;
        this->aics[param_1].SortieUnitMelee = 0x1a;
        this->aics[param_1].RaidUnit1 = 0x1a;
        this->aics[param_1].AttForceSupportAllyThreshold = 10;
        this->aics[param_1].AttForceRallyPercentage = 0x3c;
        this->aics[param_1].AttMovingTribesThreshold = 100;
        this->aics[param_1].AttAssaultDelay = 5;
        this->aics[param_1].AttUnitPatrolRecommandDelay = 0xf;
        this->aics[param_1].AttCoordinationPreference = 0x14;
        this->aics[param_1].CowThrowInterval = 0x1e;
        this->aics[param_1].SiegeEngine1 = 0x27;
        this->aics[param_1].SiegeEngine2 = 0x27;
        this->aics[param_1].SiegeEngine3 = 0x27;
        this->aics[param_1].SiegeEngine4 = 0x3b;
        this->aics[param_1].AttMaxEngineers = 10;
        this->aics[param_1].AttDiggingUnit = 0x1a;
        this->aics[param_1].AttDiggingUnitMax = 10;
        this->aics[param_1].AttMaxAssassins = 0;
        this->aics[param_1].AttUnit2 = 0;
        this->aics[param_1].AttUnit2Max = 0;
        this->aics[param_1].AttMaxLaddermen = 0;
        this->aics[param_1].AttMaxTunnelers = 0;
        this->aics[param_1].AttUnitPatrol = 0;
        this->aics[param_1].AttUnitPatrolMax = 0;
        this->aics[param_1].AttUnitPatrolGroupsCount = 0;
        this->aics[param_1].AttUnitBackup = 0x17;
        this->aics[param_1].AttUnitBackupMax = 5;
        this->aics[param_1].AttUnitBackupGroupsCount = 1;
        this->aics[param_1].AttUnitEngageMax = 0;
        this->aics[param_1].AttUnitEngage = 0;
        this->aics[param_1].AttUnitSiegeDef = 0x1a;
        this->aics[param_1].AttUnitSiegeDefMax = 0;
        this->aics[param_1].AttUnitSiegeDefGroupsCount = 0;
        this->aics[param_1].AttUnitMain1 = 0x1a;
        this->aics[param_1].AttMaxDefault = 200;
        this->aics[param_1].AttMainGroupsCount = 2;
        this->aics[param_1].TargetChoice = AITC_GOLD;
    }
}
}
