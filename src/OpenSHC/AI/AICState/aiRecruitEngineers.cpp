...
namespace AI {
    ...
    void AICState::aiRecruitEngineers(int playerID)
    {
        ...
        if ((((_aiType != AITA_NULL) && (DAT_GameState::instance.playerDataArray[playerID].isEngineerRequired != FALSE))
                && (DAT_GameState::instance.playerDataArray[playerID].canStartSpending != 0))
            && (((0x1d < (int)this->aics[_aiType - 1].recruitGoldThreshold
                             + DAT_GameState::instance.playerDataArray[playerID].currentResources[0xf]
                     && (recruitmentBuilding = DAT_GameState::instance.playerDataArray[playerID].engineersGuild.id,
                         0 < recruitmentBuilding))
                && (DAT_BuildingsState::instance.buildings[recruitmentBuilding].buildingType
                    == Map::Buildings::BT_ENGINEERSGUILD)))) {
            MACRO_CALL_MEMBER(Map::Units::UnitsState_Func::nonEuroRecruit, DAT_UnitsState::ptr)(
                Map::Units::UT_E_ENGINEER, recruitmentBuilding, playerID, 0);
