...
namespace AI {
    ...
    void AICState::playTauntToAttackedPlayerBikFromPlayer(int playerID)
    {
        ...
        if (iVar1 != 0) {
            ...
            if (DAT_GameState::instance.playerDataArray[playerID].attackedPlayerID == DAT_GameSynchronyState::instance.currentPlayerSlotID) {
                if (iVar1 == 0) {
                    iVar1 = ((byte)SEC_RNG::instance.currentNumber2 & 3) + 1;
                }
                MACRO_CALL_MEMBER(Rendering::Bink::AIMessageQueue_Func::playBikVideoFromPlayer, DAT_VideoBikQueue::ptr)(playerID, (int)DAT_GameState::instance.playerDataArray[playerID].aiType - 1, iVar1);
            }
        }
    }
}
