    ...
    void AIVState::updateBuildingsStateAndUpdateAIBuildingDecisions()
    {
        ...
        if (DAT_GameState::instance.mapAndTime.startOfDay) {
            ...
            for (int player = 1; player < 9; ++player) {
                ...
                if (DAT_GameState::instance.playerDataArray[player].keepEnclosementRelatedCountdown > 0
                    && --DAT_GameState::instance.playerDataArray[player].keepEnclosementRelatedCountdown == 0
                    && DAT_GameState::instance.playerDataArray[player].defensesDamagedByPlayer
        ...
        if (DAT_GameState::instance.mapAndTime.weekChanged) {
            MACRO_CALL_MEMBER(Map::Buildings::BuildingsState_Func::updatePathLinkageTileMap, DAT_BuildingsState::ptr)(
                2);
            for (int player = 1; player < 9; ++player) {
                if (DAT_GameSynchronyState::instance.currentPlayerFullIDArray[player] == -1
                    && DAT_GameSynchronyState::instance.currentAIArray[player] != 0
                    && DAT_GameState::instance.playerDataArray[player].playerDeathRelated == 0
                    && DAT_GameState::instance.playerDataArray[player].lordKilledByPlayerID == 0
                    && DAT_GameState::instance.playerDataArray[player].someCount31 == 0) {
        ...
        for (int player = 1; player < 9; ++player) {
            if (DAT_GameSynchronyState::instance.currentPlayerFullIDArray[player] == -1
                ...
                && DAT_GameState::instance.playerDataArray[player].playerDeathRelated == 0
                && DAT_GameState::instance.playerDataArray[player].lordKilledByPlayerID == 0
                && DAT_GameState::instance.mapAndTime.dayTicks == player) {
                MACRO_CALL_MEMBER(AIVState_Func::aiDecideOnNewBuildings, this)(player);
                MACRO_CALL_MEMBER(AICState_Func::planToBuyWhenLowOnResourceAndSnoozeBuildings, DAT_AICState::ptr)(
                    player);
            }
        }
        if (DAT_GameState::instance.mapAndTime.dayTicks == 8) {
            MACRO_CALL_MEMBER(
                Map::Buildings::BuildingsState_Func::applySnoozedStateBasedOnPlayerData, DAT_BuildingsState::ptr)();
        }
